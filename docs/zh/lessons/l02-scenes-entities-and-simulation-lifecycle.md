...
逆运动学与相机，以及 L07 的完整抓取场景都会沿用它。本讲刻意只使用 Genesis
...
配套的可执行 notebook 已经实现下面的实验。运行第一个代码 cell 前，学习者先
预测 build 边界、哪些状态值会变化，以及没有相机时什么证据仍能说明物体发生了
运动；最后再根据对比图检查这些预测。
...
notebook 先记录初始位置、四元数和线速度。启用渲染时，它还会在推进 20 步前
验证并保存初始相机画面；随后记录最终状态，检查 shape 和有限性，并验证方向性
预测 `final_z < initial_z`，而不是断言一个编造出来的绝对坐标。

### 阶段 5——如实观测

启用渲染时，notebook 会验证初始和最终两份 RGB 数组，再把两张相机画面与各自
实测高度并排显示。关闭渲染时，则把基于状态绘制的 Box 初始/最终对比图与高度
轨迹并排显示，并明确标注这不是相机图像。两个分支之后都会用反思问题引导学习者
区分状态证据与渲染证据，并将实际结果和预测进行比较。
...
- 启用渲染时验证初始和最终 RGB 数据；关闭渲染时明确报告 `SKIP`，并且只生成
  有清楚标注的状态对比图和轨迹图。

L02 已记录的验证在参考机的多个独立干净 kernel 中运行同一本 notebook：默认
`auto` 选择 `gs.amdgpu`，强制 `cpu` 选择 `gs.cpu`。启用渲染的参考运行提供额外
证据，但不会把 GPU 变成核心课程的先决条件。
...
不应出现额外的环境维度。L06 会有意引入批量环境，并解释相应的 shape。
